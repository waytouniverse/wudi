...
本仓库用于**分发安装包**（源码不公开）。macOS 与 Windows 两平台安装包见下方 [下载安装](#下载安装)。
...
自 **1.0.0** 起，无敌采用[**专有许可**](LICENSE)，保留所有权利。

- ✅ **可以**：在自己的设备上安装、运行官方构建的安装包，免费使用。
- ❌ **不可以**：复制、分发、修改、制作衍生作品、反向工程、商用，或移除版权声明。
- ℹ️ 本软件建立在多个开源项目之上，**其上游组件仍按其原有许可授权**，你依其取得的权利不因本协议而减损。

> ⚠️ 历史上曾以 MIT 协议公开分发过 v1.1.0 – v1.3.3，**那些版本已全部下架**。已经拿到旧版本的用户，其依 MIT 取得的授权不受影响、不可撤销。
